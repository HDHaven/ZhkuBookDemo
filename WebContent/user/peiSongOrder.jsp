...
<body>
	...
	<div id="personaldetil">
		...
		<div id="order">
			<a href="GetOrderInfo">全部订单</a> <a href="GetOrderInfo?orderState=待处理">待处理</a>
			<a href="GetOrderInfo?orderState=正在配送"><span
				style="color: red; font-weight: bold">正在配送</span></a> <a
		...
		<div id="orderhead">
			<table class="tb1">
				...
				<c:if test="${OrderInfo.data == null }">
					<tr>
						<td><h4 align="center">暂时没有正在配送的订单</h4>
						<td>
					</tr>
				</c:if>
			</table>
			...
			<div id="orderdetile">
					...
					<c:forEach var="pro" items="${OrderInfo.data }">
						<tr>
							...
							<td class="tb1_td6">
								...
								<a href="UpdateOrder?productId=${pro.productId }" onclick="return confirm('确定收货吗？')">确认收货</a>
							</td>
						</tr>
					</c:forEach>
				</table>
				<p align="center">
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="GetOrderByState?page=1&orderState=正在配送">首页</a>
							<a
								href="GetOrderByState?page=${OrderInfo.curPage-1 }&orderState=正在配送">上一页</a>
						</c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${OrderInfo.curPage == OrderInfo.totalPages }">下一页 尾页</c:when>
						<c:otherwise>
							<a href="GetOrder?page=${OrderInfo.curPage+1 }&orderState=正在配送">下一页</a>
							<a href="GetOrder?page=${OrderInfo.totalPages }&orderState=正在配送">尾页</a>
						</c:otherwise>
					</c:choose>
				</p>
			</div>
