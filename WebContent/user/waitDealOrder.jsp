...
<body>
	...
	<div id="personaldetil">
		...
		<div id="order">
			<a href="GetOrderInfo">全部订单</a> <a href="GetOrderInfo?orderState=待处理"><span
				style="color: red; font-weight: bold">待处理</span></a> <a
				...
				<table class="tb2">
					<c:forEach var="pro" items="${OrderInfo.data }">
						<tr>
							...
							<td class="tb1_td6"><a
								href="GetOrderDetailss?orderId=${pro.orderId }">详情</a></td>
						</tr>
					</c:forEach>
				</table>
				<p align="center">
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="GetOrderByState?page=1&orderState=待处理">首页</a>
							<a
								href="GetOrderByState?page=${OrderInfo.curPage-1 }&orderState=待处理">上一页</a>
						</c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${OrderInfo.curPage == OrderInfo.totalPages }">下一页 尾页</c:when>
						<c:otherwise>
							<a href="GetOrder?page=${OrderInfo.curPage+1 }&orderState=待处理">下一页</a>
							<a href="GetOrder?page=${OrderInfo.totalPages }&orderState=待处理">尾页</a>
						</c:otherwise>
					</c:choose>
				</p>
			</div>
